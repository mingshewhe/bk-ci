...
data class TemplateInstanceField(
    // 是否覆盖全部参数,ui方式新增的实例化,值为true,code方式新增的实例化,值为false
    // true: 全部使用流水线自定义设置,excludeParamIds,使用模版设置
    // false: 全部使用模版设置,excludeParamIds,使用流水线自定义
    val overrideAllParam: Boolean = true,
    // overrideAllParam后,需要排除的参数
    val excludeParamIds: List<String>? = null,
    // 流水线指定的自定义的触发器,只能自定义启用/禁用,不能新增修改删除触发器
    val triggerStepIds: List<String>? = null,
    // 是否覆盖全部设置,
    // true: 全部使用流水线自定义设置,excludeSettingGroups中的设置除外,使用模版设置
    // false: 全部使用模版设置,excludeSettingGroups中的设置除外,使用流水线自定义
    val overrideAllSettingGroup: Boolean = true,
    // overrideAllSettingGroup后,需要排除的设置
    ...
    fun overrideSetting(settingGroup: PipelineSettingGroupType): Boolean {
        return if (overrideAllSettingGroup) {
